    ...
    public class PhysBonePlugin : MonoBehaviour, VNyanInterface.IButtonClickedHandler
    {
        ...
        void OnImportColliders()
        {
            try
            {
                ...
                // A chain only collides with colliders named in its own list, so attach the
                // imported set to every chain — otherwise the import has no effect.
                int attached = 0;
                if (config.chains != null)
                {
                    foreach (ChainConfig chc in config.chains)
                    {
                        if (chc == null) continue;
                        if (chc.colliders == null) chc.colliders = new List<string>();
                        HashSet<string> have = new HashSet<string>(chc.colliders, StringComparer.OrdinalIgnoreCase);
                        foreach (string cn in importedNames)
                            if (have.Add(cn)) { chc.colliders.Add(cn); attached++; }
                    }
                }
                ...
                if (attached > 0) msg += ", attached to all chains";
                msg += " — Save to keep";
                SetStatus(msg);
            }
            catch (Exception e)
        ...
        void OnClearColliders()
        {
            ...
            if (config.chains != null)
                foreach (ChainConfig chc in config.chains)
                    if (chc != null && chc.colliders != null) chc.colliders.Clear();
            RebindAll();
            SetStatus("Removed " + n + " collider(s) from all chains — Save to keep");
        }
        ...
        void CreateGLMat()
        {
            ...
            if (sh == null) { Debug.LogWarning("[PhysBones] No GL shader found — collider viz unavailable"); return; }
            ...
            Debug.Log("[PhysBones] GL collider material using shader: " + sh.name);
        }
        ...
        void OnToggleColliders()
        {
            ...
            Debug.Log("[PhysBones] ShowColliders toggled -> " + showColliders

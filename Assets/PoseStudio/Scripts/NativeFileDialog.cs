...
namespace PoseStudio
{
    internal static class NativeFileDialog
    {
        ...
        public static string OpenFile(string title, string initialDir, string filterLabel, string filterPattern)
        {
            ...
            {
                Debug.LogWarning("[PoseStudio] Open dialog unavailable: " + e.Message);
                return null;
            }
            finally
            {
        ...
        public static string SaveFile(string title, string defaultName, string defExt,
                                      string filterLabel, string filterPattern)
        {
            ...
            try
            {
                // seed the filename buffer with the default name
                string seed = defaultName ?? "";
                byte[] seedBytes = Encoding.Unicode.GetBytes(seed);
                Marshal.Copy(seedBytes, 0, fileBuf, seedBytes.Length);
                Marshal.WriteInt16(fileBuf, seedBytes.Length, 0);
            ...
            {
                Debug.LogWarning("[PoseStudio] Save dialog unavailable: " + e.Message);
                return null;
            }
            finally
            {
